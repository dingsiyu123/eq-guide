...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = `
    【指令】我正在和你玩一个角色扮演游戏，你需要根据剧本，对我的心情和游戏结果进行最终裁决。
    ...
    - 好感度机制: 你说话得体，我的好感度+1~2分；你说话冒犯，我的好感度-20~30分。

    【新增：师爷打分规则】  ⬅️ 这是新加的
    - 如果玩家**输了**：score 必须在 20-59 之间，根据玩家表现的糟糕程度给分
    - 如果玩家**赢了**：score 必须在 61-100 之间，根据玩家的巧妙程度给分
    - 60分是临界点（平局），但游戏中不会出现平局
    - 注意：isWin=true 时，score 必须 > 60；isWin=false 时，score 必须 < 60

    【输出格式】
    你必须只输出一个JSON对象，包含我的新好感度、游戏是否结束、你是否胜利、**师爷打分**、点评，以及一句幽默的对方最终行动。
    {
      "mood": <我的新好感度>,
      "isGameOver": <true/false>,
      "isWin": <true/false>,
      "score": <师爷打分，输了20-59分，赢了61-100分>,  ⬅️ 新增字段
